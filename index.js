...
const uri = `mongodb+srv://${username}:${password}@cluster0.yexdchm.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.post("/chocolate", async (req, res) => {
      ...
      res.send(result);
      // console.log("New Chocolate: ", newChocolate);
    });

    app.get("/chocolate/:id", async (req, res) => {
      const id = req.params.id;
      console.log("Chocolate get: ", id);
...
app.listen(port, () => {
  console.log(`Chocolate is running on port: ${port}`);
});
